...
import time
"""
class MuDummy2(PlayerNetwork, ABC):
...
class MuZero:
    ...
    def __init__(self, game_name, config=None, split_resources_in=1):
        ...
        #self.player1 = MuPlayer(avatar=180, battle_format="gen8randombattle")
        #self.player2 = MuPlayer(avatar=181, battle_format="gen8randombattle")
    ...
    def terminate_workers(self):
        ...
        if self.shared_storage_worker:
            ...
            #self.checkpoint = await self.shared_storage_worker.get_checkpoint.remote()
        if self.replay_buffer_worker:
            self.replay_buffer = ray.get(self.replay_buffer_worker.get_buffer.remote())
            #self.replay_buffer = await self.replay_buffer_worker.get_buffer.remote()
    ...
    async def test(
        ...
        for i in range(num_tests):
            ...
            results.append(
                ...
                #    )
                #)
                self_play_worker.play_game.remote(0, 0, render, opponent, muzero_player)
            )
    ...
    try:
        ...
        while 0 < budget or any(running_experiments):
            for i, experiment in enumerate(running_experiments):
                #if experiment and experiment.config.training_steps <= await
