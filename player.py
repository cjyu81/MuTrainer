...
class Player(PlayerNetwork, ABC):
    ...
    async def player_message(
        ...
        print("player message ran method")
        await self._logged_in.wait()
        self.logger.info("Event logged in attempt of sending message")
        start_time = perf_counter()
        await self._send_message(f"/pm {username}, {message}")
        self.logger.info(
            "Message sent in %fs",
    ...
    async def _create_battle(self, split_message: List[str]) -> Battle:
        ...
        :rtype: Battle
        """
        #print("Charlie split message player 146", split_message)
    ...
    async def _get_battle(self, battle_number: str) -> Battle:
        #print("Charlie battle number player 180", battle_number)
        while True:
    ...
    async def _handle_battle_request(
        ...
        if maybe_default_order and random.random() < self.DEFAULT_CHOICE_CHANCE:
            print("RANDOMLY HANDLED DEFAULT CHOICE SELECTED")
