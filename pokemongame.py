...
class Game(AbstractGame):
    ...
    async def play_battle(self, gh1, gh2, config, model, temp, thresh):
        await self.env.play_battle(gh1, gh2, config, model, temp, thresh)
...
class PokemonGame:
    """
    Controls two mu_player classes to battle against each other
    Acts as a hub for two to send results to self_play
    Playerone challenges, Playertwo accepts.
    Mu_player inherently keeps its own gamehistory and it is requested here.
    Playerone returns the end result and closing.
    gh1 and gh2 are passed as references through parameters. They are edited accordingly
    """
    def __init__(self, seed=None,playeronenum=0,playertwonum=0):
    ...
    async def play_battle(self, gh1, gh2, config, model, temp, thresh):
        print("pokegame 253 play_battle called")
        #battle beginning assignemnt
        #await asyncio.gather(self.reset())#This doesnt do anything Im just keeping it for asthetic
        #supposed to be to_play but to_play is not in synchrnous
        self.player1.gh = gh1
        self.player2.gh = gh2
        await self.player1.battle_once(self.player2)
    ...
    async def reset(self):
        """
        Set up new battle for 2 player objects.
        Randomly seleect moves to end any current battles if in progress.
        """
