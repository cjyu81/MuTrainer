        ...
        while ray.get(
            ...
            if not test_mode:
                ...
                shared_storage.set_info.remote(
                    {
                        "episode_length": len(gamehistory1.action_history) - 1,
                        "total_reward": sum(gamehistory1.reward_history),
                        "mean_value": numpy.mean(
                            [value for value in gamehistory1.root_values if value]
                        ),
                    }
                )
                shared_storage.set_info.remote(
                    {
                        "episode_length": len(gamehistory2.action_history) - 1,
                        "total_reward": sum(gamehistory2.reward_history),
                        "mean_value": numpy.mean(
                            [value for value in gamehistory2.root_values if value]
                        ),
                    }
                )
                #Not using because game has imperfect information
                if 1 < len(self.config.players):
                    shared_storage.set_info.remote(
                        {
                            "muzero_reward": sum(
                                reward
                                for i, reward in enumerate(gamehistory1.reward_history)
                                if gamehistory1.to_play_history[i - 1]
                                == self.config.muzero_player
                            ),
                            "opponent_reward": sum(
                                reward
                                for i, reward in enumerate(gamehistory2.reward_history)
                                if gamehistory2.to_play_history[i - 1]
                                != self.config.muzero_player
                            ),
                        }
                    )
