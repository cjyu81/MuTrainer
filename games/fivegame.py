...
class FiveGame:
    ...
    def __init__(self, seed=None):
        ...
        self.visible = numpy.zeros((5), dtype="int32")
    ...
    def step(self, action):
        #get battle object
        #use action in battle objects
        #if move fails, re-call decision making process
        #if move works, proceed
        #return get_observation, reward, done
        value = 1
        if action< 5:
            self.visible[action] +=value
        elif action == 5:
            self.visible[0]+= value
            self.visible[1] += value
        elif action == 6:
            self.visible[1]+= value
            self.visible[2]+= value
        elif action == 7:
            self.visible[2]+= value
            self.visible[3] += value
    ...
    def reset(self):
        #get account objects and terminate current battles?
        #issue new battle challenge
        self.board = numpy.zeros((5), dtype="int32")
        self.player = 1
